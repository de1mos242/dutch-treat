...
class BalanceService {
    ...
    fun calculateBalance(event: Event): List<BalanceRow> {
        ...
        return event.participants.map {
            val purchases =
                event.purchases?.filter { p -> p.buyerId == it.id }?.map { p -> p.amount }?.plus(BigDecimal.ZERO)
                    ?.reduce(BigDecimal::add)
                    ?: BigDecimal.ZERO

            val consumes =
                event.purchases?.filter { p -> p.consumerIds == null || p.consumerIds.contains(it.id) }?.map { p ->
                    p.amount.divide(
                        if (p.consumerIds == null) participantsCount else p.consumerIds.size.toBigDecimal(),
                        4,
            ...
            val sends =
                event.transfers?.filter { t -> t.senderId == it.id }?.map { t -> t.amount }?.plus(BigDecimal.ZERO)
                    ?.reduce(BigDecimal::add)
                    ?: BigDecimal.ZERO

            val receives =
                event.transfers?.filter { t -> t.receiverId == it.id }?.map { t -> t.amount }?.plus(BigDecimal.ZERO)
                    ?.reduce(BigDecimal::add)
                    ?: BigDecimal.ZERO
